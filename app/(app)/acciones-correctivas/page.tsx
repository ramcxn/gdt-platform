...
export default async function Page() {
  ...
  const { data: rows } = await supabase.from('acciones_correctivas').select('*').eq('empresa_id', eid).order('created_at', { ascending: false }).limit(50)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-bold text-white">Acciones Correctivas</h1><p className="text-slate-400 text-sm">Seguimiento de acciones correctivas y preventivas</p></div>
        </div>
      ...
      <div className="glass-card rounded-xl border border-white/5 overflow-hidden" style={{background:'rgba(15,31,53,0.7)'}}>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-white/5">{['Título','Área','Responsable','Estado','Fecha'].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-white/5">
            {rows?.length ? (rows as any[]).map(r=>(
              <tr key={r.id} className="hover:bg-white/3 transition-colors">
                <td className="px-4 py-3 text-white font-medium max-w-[160px] truncate">{r.titulo??'—'}</td>
                <td className="px-4 py-3 text-slate-300">{r.area??'—'}</td>
                <td className="px-4 py-3 text-slate-300">{r.responsable??'—'}</td>
            ...
            )) : <tr><td colSpan={5} className="px-4 py-16 text-center"><div className="text-4xl mb-3">✅</div><p className="text-slate-400">No hay registros en Acciones Correctivas</p></td></tr>}
          </tbody>
        </table>
      </div>
    </div>
