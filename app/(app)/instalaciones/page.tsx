...
export default async function InstalacionesPage() {
  ...
  const { data: rows } = await supabase.from('mantenimiento_instalaciones').select('*').eq('empresa_id', eid).order('created_at', { ascending: false }).limit(50)
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {[{i:'🏗️',l:'Total trabajos',v:rows?.length??0},{i:'🔧',l:'En proceso',v:rows?.filter(r=>r.estado==='En_Proceso').length??0},{i:'✅',l:'Completados',v:rows?.filter(r=>r.estado==='Completado').length??0}].map((k,i)=>(
      ...
      <div className="glass-card rounded-xl border border-white/5 overflow-hidden" style={{background:'rgba(15,31,53,0.7)'}}>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-white/5">{['Área','Tipo','Descripción','Responsable','Fecha prog.','Estado'].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-white/5">
            {rows?.length ? rows.map(r=>(
              <tr key={r.id} className="hover:bg-white/3"><td className="px-4 py-3 text-white font-medium">{r.area||'—'}</td><td className="px-4 py-3 text-slate-300">{r.tipo}</td><td className="px-4 py-3 text-slate-300 max-w-[180px] truncate">{r.descripcion||'—'}</td><td className="px-4 py-3 text-slate-300">{r.responsable||'—'}</td><td className="px-4 py-3 text-slate-400 text-xs">{r.fecha_programada||'—'}</td><td className="px-4 py-3"><Badge text={r.estado} color={eColor[r.estado]??'slate'}/></td></tr>
            )) : <tr><td colSpan={6} className="px-4 py-16 text-center"><div className="text-4xl mb-3">🏗️</div><p className="text-slate-400">No hay trabajos registrados</p></td></tr>}
          </tbody>
        </table>
      </div>
    </div>
