    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-bold text-white">Antidoping</h1><p className="text-slate-400 text-sm">Control de pruebas toxicológicas</p></div>
        </div>
      ...
      {positivos > 0 && <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-center gap-3"><span className="text-2xl">🚨</span><div><p className="text-red-300 font-semibold text-sm">{positivos} resultado(s) positivo(s)</p><p className="text-red-400/70 text-xs mt-0.5">Requiere protocolo de acción inmediata</p></div></div>}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[{i:'🧪',l:'Total pruebas',v:rows?.length??0},{i:'✅',l:'Negativos',v:rows?.filter(r=>r.resultado==='Negativo').length??0},{i:'🚨',l:'Positivos',v:positivos},{i:'🏥',l:'Laboratorios',v:new Set(rows?.map(r=>r.laboratorio).filter(Boolean)).size}].map((k,i)=>(
      ...
      <div className="glass-card rounded-xl border border-white/5 overflow-hidden" style={{background:'rgba(15,31,53,0.7)'}}>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-white/5">{['Empleado','Puesto','Fecha','Resultado','Laboratorio','Observaciones'].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-white/5">
            {rows?.length ? rows.map(r=>(
              <tr key={r.id} className="hover:bg-white/3"><td className="px-4 py-3 text-white font-medium">{r.nombre_empleado||'—'}</td><td className="px-4 py-3 text-slate-300">{r.puesto||'—'}</td><td className="px-4 py-3 text-slate-300 text-xs">{new Date(r.fecha).toLocaleDateString('es-MX')}</td><td className="px-4 py-3"><Badge text={r.resultado} color={r.resultado==='Positivo'?'red':r.resultado==='Rehusado'?'amber':'green'}/></td><td className="px-4 py-3 text-slate-300">{r.laboratorio||'—'}</td><td className="px-4 py-3 text-slate-400 text-xs">{r.observaciones||'—'}</td></tr>
