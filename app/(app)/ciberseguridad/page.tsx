...
export default async function Page() {
  ...
  const { data: rows } = await supabase.from('ciberseguridad').select('*').eq('empresa_id', eid).order('created_at', { ascending: false }).limit(50)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl text-xl flex items-center justify-center flex-shrink-0" style={{background:'linear-gradient(135deg,#1E3A5F,#2E6DA4)'}}>🔐</div>
          <div><h1 className="text-xl font-bold text-white">Ciberseguridad</h1><p className="text-slate-400 text-sm">Control de incidentes y seguridad digital</p></div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {[{i:'🔐',l:'Total registros',v:rows?.length??0},{i:'📅',l:'Este mes',v:thisMo},{i:'🕐',l:'Último registro',v:rows?.[0]?new Date((rows[0] as any).created_at).toLocaleDateString('es-MX',{day:'numeric',month:'short'}):'—'}].map((k,i)=>(
      ...
      <div className="glass-card rounded-xl border border-white/5 overflow-hidden" style={{background:'rgba(15,31,53,0.7)'}}>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-white/5">{['Evento','Severidad','Estado','Responsable','Fecha'].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-white/5">
            {rows?.length ? (rows as any[]).map(r=>(
              <tr key={r.id} className="hover:bg-white/3 transition-colors">
                <td className="px-4 py-3 text-white font-medium max-w-[160px] truncate">{r.tipo_evento??'—'}</td>
                <td className="px-4 py-3 text-slate-300">{r.severidad??'—'}</td>
                <td className="px-4 py-3 text-slate-300">{r.estado??'—'}</td>
                <td className="px-4 py-3"><Badge text={String(r.responsable??'—')} color={riskColor[r.responsable]??'slate'}/></td>
                <td className="px-4 py-3 text-slate-500 text-xs">{new Date(r.created_at).toLocaleDateString('es-MX')}</td>
              </tr>
            )) : <tr><td colSpan={5} className="px-4 py-16 text-center"><div className="text-4xl mb-3">🔐</div><p className="text-slate-400">No hay registros en Ciberseguridad</p></td></tr>}
          </tbody>
        </table>
      </div>
    </div>
